...
*/
			...
			// działa zajebiście
			{
				var exp = Expression.ParseGetter<PromoContext, bool>("_Attr.Has('CurrentReceipt.TotalQuantityForArticleIndex0000125837')");
				var result = exp.GetValue(promoContext);
                Console.WriteLine($"exp.GetValue<bool, PromoContext>(promoContext) = {result}");
			}

			{
				var exp = Expression.ParseGetter<PromoContext, decimal>("_Attr.Has('CurrentReceipt.TotalQuantityForArticleIndex0000125837') ? 1m : 0m");
