...
#if NETFRAMEWORK
using System.EnterpriseServices;    // unused, and has no counterpart outside .NET Framework
#endif
using System.Globalization;
using System.IO;
using System.Reflection;
#if SPRING_EXPRESSIONS_SERIALIZABLE
...
#if NETFRAMEWORK
using System.Web.Services;          // unused, and has no counterpart outside .NET Framework
#endif
...
//            ContextRegistry.Clear();
        ...
        // Compiled out rather than deleted, so the record of what the fork dropped stays visible.
        // Expressions are no longer serializable: [Serializable] and the ISerializable plumbing came off
        // SpringAST and 61 node classes, so the SerializationInfo constructor below has no base to chain
        // to and this region cannot compile. BinaryFormatter is also gone from .NET 9+ and SoapFormatter
        // never existed outside .NET Framework, so there is no target left on which these three tests
        // could run. The symbol below is deliberately never defined anywhere.
#if SPRING_EXPRESSIONS_SERIALIZABLE
...
//            Assert.AreEqual(typeof(int?), ExpressionEvaluator.GetValue(null, "T(System.Nullable`1[[System.Int32, mscorlib]], mscorlib)"));
        ...
        public void TestConstructor()
        {
            ...
            Assert.AreEqual(new DateTime(2004, 8, 14), exp.GetValue<object>(null));
        ...
        {
            // WebMethodAttribute (System.Web.Services) and TransactionOption (System.EnterpriseServices)
            // exist only on .NET Framework, so the attribute syntax is exercised there against those and
            // everywhere against [Serializable]. Nothing about the expression engine differs.
#if NETFRAMEWORK
