...
namespace SpringExpressions
{
    ...
    public class FunctionNodeTests
    {
        ...
        public void ExecutesDelegatePerformance()
        {
            ...
            // Wrapped once outside the loop on purpose: Expression.Wrap builds an evaluator, so wrapping
            // per iteration would time the wrapping rather than the evaluation this test is measuring.
            IExpression fnExp = Expression.Wrap(fn);
            using (watch.Start("Duration SpEL: {0}"))
            {
                for (int i = 0; i < ITERATIONS; i++)
                {
                    fnExp.GetValue<object>(null, vars);
                }
            }
        }
    }
